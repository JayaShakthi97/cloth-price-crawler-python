...
from simple_scrapy.items import ClothItem
...
class FashionbugSpider(CrawlSpider):
    ...
    def parse_item(self, response):
        for cloth in response.selector.xpath("//li[contains(@class, 'product type-product')]"):
            loader = ItemLoader(
                item=ClothItem(), selector=cloth, response=response)
            loader.add_xpath(
                'title',
